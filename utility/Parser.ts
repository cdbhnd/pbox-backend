...
export class Parser {
    ...
    protected typeCast(val) {
        if (!this.options.typeCast) return val;
        if (val === String(this.options.parseNum(val))) return this.options.parseNum(val);
        if (moment(val, this.options.dateFormat, true).isValid()) return this.options.parseDate(val);
        return val;
    }
}